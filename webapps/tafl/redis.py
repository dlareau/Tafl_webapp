...
def send_win(winner, loser):
    winner_redis_publisher = RedisPublisher(facility='game_move',
             users=[winner.user.username])
    loser_redis_publisher = RedisPublisher(facility='game_move',
             users=[loser.user.username])
    message = {}
    message['win'] = True
    message = RedisMessage(json.dumps(message))
    winner_redis_publisher.publish_message(message)
    message = {}
    message['lose'] = True
    message = RedisMessage(json.dumps(message))
    loser_redis_publisher.publish_message(message)
